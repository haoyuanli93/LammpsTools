...
parser.add_argument("t", help="Reduced temperature: (T-Tc)/Tc", type=float)
parser.add_argument("randomSeed", help="Reduced temperature", type=int)

args = parser.parse_args()

Tc = 640  # The critical temperature of the TIP4P/2005 model
...
def initializeLammpsScriptNVT(fileName, temperature, randomSeed):
    with open(fileName, 'w') as lammpsScript:
        ...
        lammpsScript.write("restart 10000 ./output/myRestart.* \n")
        lammpsScript.write("run   40000 \n")


def restartLammpsScriptNVT(fileName, temperature, getRDF=False, saveAtomPosition=False, runNum=40000, ):
    with open(fileName, 'w') as lammpsScript:
        # Initilization
        lammpsScript.write("#   Load system information \n")
        lammpsScript.write("include \"system.in.init\" \n")
        lammpsScript.write("read_restart \"./output/myRestart.20000\" \n")
        lammpsScript.write("include \"system.in.settings\" \n")
        lammpsScript.write("\n")
...
def createNewProj(projID, temperature=293.15, densityGCm3=0.789, molNum=1024, randomSeed=1000,
    ...
    os.system("/global/cfs/cdirs/lcls/xpp/software/moltemplate/moltemplate/scripts/moltemplate.sh system.lt")
    os.mkdir("./logFiles")
    os.mkdir("./output")
    os.chdir("../")
...
label = "{:.1f}".format(args.t)
label.replace('.', 'p')

createNewProj("_{}Tc".format(label),
              temperature=args.t * Tc,
              densityGCm3=0.31,
              molNum=100 * 1000,
